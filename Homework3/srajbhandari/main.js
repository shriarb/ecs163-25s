...
// defining a consistent color scale for exercise frequency categories using d3.schemeSet2
const color = d3.scaleOrdinal()
  .domain(frequencyTypes)
  .range(d3.schemeSet2.slice(0, frequencyTypes.length));
...
d3.csv("data/fitness_cleaned.csv").then(data => {
    ...
    grouped.forEach(([gender, ageMap]) => {
      ageMap.forEach(([age, freqMap]) => {
          freqMap.forEach(([freq, count]) => {
              processedData.push({ gender, age, freq, count });
          });
      });
    });
  ...
  // Settin up interaction for the gender filter dropdown
  d3.select("#genderDropdown").on("change", function () {
    ...
    drawBarChart(filtered); // Redraw bar chart only
  });

  function drawBarChart(data) {
    // Clearing any existing SVG elements inside the bar chart container
    ...
    // Setting up scales
    // x0: outer band scale for age groups (main x-axis categories)
    const x0 = d3.scaleBand().domain(ages).range([0, width]).padding(0.2);

    // x1: inner band scale for frequencies within each age group
    const x1 = d3.scaleBand().domain(freqs).range([0, x0.bandwidth()]).padding(0.05); 

    // y: linear scale for bar height (number of respondents)
    const y = d3.scaleLinear().domain([0, d3.max(data, d => d.count)]).nice().range([height, 0]);

    // X axes
    g.append("g")
      .attr("transform", `translate(0,${height})`)
      .call(d3.axisBottom(x0)) // Rendering the x-axis at the bottom using the x0 scale (age groups)
      .selectAll("text")
    ...
    // Y axis
    // Appendinf and renderring the y-axis on the left using the linear y scale (respondent count)
    ...
    const bars = newGroups.append("rect")
      .attr("x", d => x1(d.freq))
    ...
    bars
      .on("mouseover", function(event, d) {
        tooltip
          ...
          .html(`<strong>Age:</strong> ${d.age}<br><strong>Freq:</strong> ${d.freq}<br><strong>Count:</strong> ${d.count}`);
      })
    ...
    bars.transition()
      .delay((d, i) => i * 50)
      .duration(600)
      // using a smooth cubic easing function for a more natural deceleration at the end of the transition
  ...
  function drawSankey(data) {  
    // Selecting the SVG element where the Sankey diagram will be rendered
  ...
  // Combining duplicate links 
    ...
    nodeRects
      .on("mouseover", function(event, d) {
        tooltip
          .style("visibility", "visible")
          .html(`<strong>${d.name}</strong><br><strong>Total Flow:</strong> ${d.value}`);
      })
      .on("mousemove", function(event) {
        tooltip
          .style("top", `${event.pageY - 40}px`)
    ...
    const linkPaths = g.append("g")
      ...
      .enter()
      .append("path")
      // Defining the path shape for each Sankey link using a horizontal curved connector
      .attr("d", d3.sankeyLinkHorizontal())
      .attr("stroke", d => nodes[d.source.index].color)
  ...
  function drawHeatmap(data) {
    // Selecting the SVG element where the heatmap (Likert chart) will be drawn
    ...
    const margin = { top: 25, right: 15, bottom: 40, left: 80 };
    const width = 600;
    const height = 320;
    ...
    // Grouping for heatmap tiles and axes, translated by margin
    ...
    // creating Y-axis
    heatmapGroup.append("g")
    // Rendering the y-axis on the left side using the y scale (Likert questions/categories)
      .call(d3.axisLeft(y))
      .attr("stroke", "#888")
      .selectAll("text")
      .style("font-family", "ui-rounded, 'Hiragino Maru Gothic ProN', Quicksand, Comfortaa, Manjari, 'Arial Rounded MT', 'Arial Rounded MT Bold', Calibri, source-sans-pro, sans-serif")
    ...
    heatmapGroup.selectAll("rect")
      ...
      .append("rect")
      .attr("class", "tile")
      .attr("x", d => x(d.response)) // response = x-axis
      .attr("y", d => y(d.question)) // question = y-axis
    ...
    // Creating a brush component for interactive selection on the heatmap
    ...
    // Selecting the element for displaying the total count of selected responses in the heatmap
    const countDisplay = d3.select("#selection-count")
    ...
    function brushed(event) {
      ...
      heatmapGroup.selectAll(".tile")
        .classed("selected", d => {
          const xPos = x(d.response);
          const yPos = y(d.question);

          const tileWidth = x.bandwidth();
          const tileHeight = y.bandwidth();
          ...
          const isSelected =
            ...
            yPos + tileHeight > y0 &&
            yPos < y1;
          if (isSelected) count += d.count;
          return isSelected;
        });
      countDisplay.text(`Total Responses in Selection: ${count}`);
    }

    // Clearing all brushed selections and reset the count display when the SVG is double-clicked
    svg.on("dblclick", () => {
      heatmapGroup.selectAll(".tile").classed("selected", false);
